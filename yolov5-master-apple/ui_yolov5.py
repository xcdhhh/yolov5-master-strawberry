...
class Ui_MainWindow(QWidget):
    def __init__(self, parent=None):
        ...
        window_pale.setBrush(self.backgroundRole(), QtGui.QBrush(
            QtGui.QPixmap(os.getcwd() + '\\background\\background.jpg')))
...
class picture(QWidget):
    def __init__(self):
        ...
        pixmap = QPixmap(os.getcwd() + '\\background\\background.jpg')
        ...
        # 创建检测图片和结果标签
        self.label1 = QLabel("   待检测图片", self)
        self.label1.setFixedSize(700, 500)
        self.label1.setStyleSheet(
            "background:#7A6969; color: rgb(255, 255, 255); font-size: 20px; font-weight: bold; font-family: 宋体;")
        self.label1.move(110, 200)

        self.label2 = QLabel("   检测结果", self)
        self.label2.setFixedSize(700, 500)
        self.label2.setStyleSheet(
            "background:#7A6969; color: rgb(255, 255, 255); font-size: 20px; font-weight: bold; font-family: 宋体;")
        self.label2.move(850, 200)

        # 创建保存路径
        camera_or_video_save_path = 'data\\test'
        os.makedirs(camera_or_video_save_path, exist_ok=True)
        # self.label3 = QLabel(self)
        # self.label3.setText("   病情分析")
        # self.label3.move(110, 740)
        # self.label3.setStyleSheet("QLabel{background:#7A6969;}"
        #                           "QLabel{color:rgb(300,300,300,120);font-size:20px;font-weight:bold;font-family:宋体;}"
        #                           )
        # self.label3.setFixedSize(1440,100 )
    ...
    def openimage(self):
        ...
        if imgName!='':
            ...
            # 调整原图像尺寸，可选择是否使用
            # resized_image = cv2.resize(self.im0, (width_new, height_new))
            # width_1 = resized_image.shape[1]
            # height_1 = resized_image.shape[0]
